...
chroot "$rootfs" ucfq relative/path.conf 2>&1 | grep -q "fully qualified path name"
...
chroot "$rootfs" ucfq /etc/present.conf | grep -q "^Configuration file"
! chroot "$rootfs" ucfq --with-colons /etc/present.conf | grep -q "^Configuration file"
