...
rootfs="$(install-slices gcc-15-powerpc64le-linux-gnu_gcc-15)"
ln -s "powerpc64le-linux-gnu-gcc-15" "${rootfs}/usr/bin/gcc"

test "$(chroot "${rootfs}" gcc -print-search-dirs | head -n 1)" = "install: /usr/lib/$gcc_dir/powerpc64le-linux-gnu/15/"
chroot "${rootfs}" gcc -print-search-dirs | head -n 2 | tail -n 1 | grep -q "/usr/libexec/$gcc_dir/powerpc64le-linux-gnu/15/"
...
touch "${rootfs}/usr/libexec/$gcc_dir/powerpc64le-linux-gnu/15/foo"
chmod +x "${rootfs}/usr/libexec/$gcc_dir/powerpc64le-linux-gnu/15/foo"

chroot "${rootfs}" gcc -print-prog-name=foo
test "$(chroot "${rootfs}" gcc -print-prog-name=foo)" = "/usr/libexec/$gcc_dir/powerpc64le-linux-gnu/15/foo"
