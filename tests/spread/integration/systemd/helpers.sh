#!/bin/bash
#spellchecker: ignore rootfs nsenter nsrun nsystemctl getty firstboot uts
...
boot_rootfs() {
  ...
  # no tty in here, and system-services enables firstboot, which would prompt
  # on a console or fail without one; the runtime masks stay out of the cut
  mkdir -p "$rootfs/run/systemd/system"
  for unit in ${BOOT_MASKS-console-getty.service systemd-firstboot.service}; do
