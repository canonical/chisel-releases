...
test "$(chroot "${rootfs}" gcc -print-search-dirs | head -n 1)" = "install: /usr/lib/$gcc_dir/aarch64-linux-gnu/14/"
chroot "${rootfs}" gcc -print-search-dirs | head -n 2 | tail -n 1 | grep -q "/usr/libexec/$gcc_dir/aarch64-linux-gnu/14/"
...
touch "${rootfs}/usr/libexec/$gcc_dir/aarch64-linux-gnu/14/foo"
chmod +x "${rootfs}/usr/libexec/$gcc_dir/aarch64-linux-gnu/14/foo"

chroot "${rootfs}" gcc -print-prog-name=foo
test "$(chroot "${rootfs}" gcc -print-prog-name=foo)" = "/usr/libexec/$gcc_dir/aarch64-linux-gnu/14/foo"
