...
chroot "$rootfs" sysctl --help 2>&1 | grep -Fiq 'usage:'
chroot "$rootfs" sysctl --version 2>&1 | grep -Fq 'sysctl from procps-ng'
...
chroot "$rootfs" sysctl -n kernel.hostname | grep -Fq 'test-hostname'
grep -Fq 'test-hostname' "$rootfs"/proc/sys/kernel/hostname
...
chroot "$rootfs" sysctl -n kernel.hostname | grep -Fq 'file-hostname'
