...
Never add `slices/` or `chisel.yaml` to `main`. If writing or modifying slice definitions or their spread tests, work on a checkout of the target release branch and follow [AGENTS-release.md](./AGENTS-release.md). That file is maintained here on `main`; each release branch carries a short `AGENTS.md` stub pointing to it.
